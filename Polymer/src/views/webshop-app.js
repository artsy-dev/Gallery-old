...
class TodoApp extends connect(store)(LitElement) {
  ...
  render() {
    return html`
      <app-drawer ?opened="${this._drawerOpened}"> </app-drawer>
  ...
  firstUpdated() {
    ...
    installMediaQueryWatcher("(max-width: 765px)", match =>
      store.dispatch(updateCompactLayout(match))
    );
  }
  ...
  updated(changedProps) {
    if (changedProps.has("_page")) {
      updateMetadata({
        title: `Gallery ${this._page.title ? " - " + this._page.title : ""}`,
